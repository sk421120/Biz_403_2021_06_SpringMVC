...
<form id="student_input" method="POST">
	<fieldset>
		<div>
			<label>학번</label>
			<input name="st_num" id="st_num"	value=<c:out value="${STUDENT.st_num}" default="0"/>
		...
		<div>
			<label>전화번호</label>
			<input name="st_tel" id="st_tel" placeholder="주소를 입력하세요">
		</div>
		<div>
			<label>주소</label>
...
const student_submit = () => {
	...
	if(st_num.value === "") {
		alert("학번은 반드시 입력하세요")
		st_num.focus()
		// event 핸들링에서 코드 진행을 멈추고자 할때 반드시
		return false;
	}
	...
	if(target.tagName === "BUTTON") {
		...
		*/
		if(target.className.includes("save")) {
			// 여기에 event 처리 코드를 작성할 수 있지만 if문 내의 코드가 다소 복잡해지므로
