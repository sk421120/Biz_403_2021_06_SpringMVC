...
public class BookController {
	...
	public BookController(BookService bkService) {
...
//		}
	...
	public String insert(@ModelAttribute BookVO vo) {
		 ...
		 * 숫자형 변수가 있을 경우 건너온 데이터는 무조건 문자열형 이므로 숫자형 형 변환을 시도한다
		 * 그런데 입력박스에 아무런 값이 담겨 있지 않는 경우는 다음과 같은 코드가 내부적으로 실행되면서
		 * 	bookVO.setBk_pages( Integer.valueOf("") );
		 * NumberFormatException이 발생하고 즉시 client에서 400 오류를 전달하고 종료
		 * Controller의 사용자 코드에 다다르기 전에 오류가 나고 method의 실행이 종료되어 손쓸 방법이 없다
		 */
