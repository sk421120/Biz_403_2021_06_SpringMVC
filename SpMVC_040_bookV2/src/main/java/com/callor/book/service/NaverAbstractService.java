...
/*
 * abstract 키워드를 추가 > 추상클래스가 된다
 * 
 * 추상클래스
 * 일반적인 클래스와 인터페이스의 성질을 모두 포함하는 클래스
 * 
 * 목적
 * 1. 상속 되는 부모 클래스의 역할 스스로는 객체를 생성될 수 없다
 ...
 * 
 * 5. 인터페이스의 역할을 수행
 * 	여러 클래스의 원형(prototype)역할을 수행하면서 상속받은 클래스들에
 * 	공통으로 필요한 코드를 미리 제공한다
 * 
 * 객체생성하는 방법
...
public abstract class NaverAbstractService<T> {
	...
	public abstract List<T> getNaverList(String jsonString) throws ParseException, Exception;
}
