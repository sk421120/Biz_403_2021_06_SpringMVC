 ...
 * 
 * json-simple, Gson 등을 사용할때는 OpenAPI를 통하여 JsonString을 수신하고
 * parsing를 통해 DTO, List<DTO>형태의 데이터로 변환하는 과정을 수행했다.
 * 
 ...
 * 
 * 내부에서 요청정보를 OpenAPI로 보내고 수신된 데이터를 DTO(VO)구조에 따라
 * 자체 parsing을 하고 결과를 돌려준다.
 */
@Slf4j
...
public class NaverNewsServiceImplV2 extends NaverNewsServiceImplV1{
	...
	public List<NewsDTO> getNaverList(String queryURL) throws Exception{
		...
		// (naver에 요청할 URL 정보, GET mehtod로 요청, 인증정보등이 담긴 header, 내가 받을 데이터)
