 ...
 * Restfull Service
 * API Service
 * Request 에 대하여 view(*.jsp, *.html)등을 보여주지 않고 약속된 데이터를 Response 서비스
 * 
 * 절대 view를 return하지 않는다
 * return 되는 type으로 데이터를 response 한다 주로 String, JSON(List, VO, DTO)형식으로 return 
 */
@Slf4j
@RestController
...
public class ApiController {
	 ...
	 * 일반적인 JSP(form, link)에서 보낸 데이터는 매개변수를 통하여 직접 전달받을 수 있는데
	 ...
	 * 
	 * 또한 pom.xml jackson-databind Dependency를 반드시 설정해 둔다
	 * 
	 */
	...
	public String reqAjax(@RequestBody Map<String,String> maps) {
		...
		log.debug("stname {} ", maps.get("stname"));
