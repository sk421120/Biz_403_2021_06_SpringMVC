...
<body>
	<nav id="main_nav">
		<c:if test="${CAT == 'BOOK'}"><c:set var="pHolder" value="도서 검색어"/></c:if>
		<c:if test="${CAT == 'MOVIE'}"><c:set var="pHolder" value="영화 검색어"/></c:if>
		<c:if test="${CAT == 'NEWS'}"><c:set var="pHolder" value="뉴스 검색어"/></c:if>
	<select name="category">
		<option value="BOOK"
			<c:if test="${CAT == 'BOOK'}">selected=selected</c:if>>도서검색</option>
		<!-- <c:if test="${pHolder == '도서 검색어'}">selected=selected</c:if> >도서검색</option>  --> 
		<option value="MOVIE"
