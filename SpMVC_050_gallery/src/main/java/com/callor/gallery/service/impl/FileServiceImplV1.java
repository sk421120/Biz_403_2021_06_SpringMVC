...
public class FileServiceImplV1 implements FileService{
	...
	// 서버의 특별한 폴더에 접근하기 위해 서버의 정보를 가져오기 위한 helper class
//	private ServletContext context;
	...
	public String fileUp(MultipartFile file) throws Exception {
		...
		// file-context.xml에 설정된 files 폴더 정보를 가져오기
...
//		log.debug("path {}", res.getURI().getPath());
		...
		/*
		 * 파일을 업로드할때 보안 주의하기 실제 파일이름으로 업로드를 수행하면
		 * 쉽게 업로드 구현이 된다
		 * 
		 * 하지만 만약 같은 이름의 파일을 중복해서 업로드하면 먼저 업로드 된 파일이
		 * 변경되는 문제가 발생한다
		 * 
		 * 해커에 의해서 같은 이름으로 파일들을 업로드 해 버리면 저장된 원래 파일들이
		 * 모두 변조될 수 있다
		 * 
		 * 이러한 문제를 방지하기 위하여 UUID 문자열을 생성하고
		 * UUID + 원래 파일이름.원래 확장자 형식으로 업로드를 수행한다
...
//		File uploadPathAndFile = new File(filePath, originalFileName);
		File uploadPathAndFile = new File(filePath, strUUID);
		
		// file에 담긴 파일정보를 사용하여 uploadPathAndFile에 전송하라
		// (복사하라, 업로드하라)
	...
	public String fileUpCount(MultipartFile file) throws Exception {
		return null;
	}
