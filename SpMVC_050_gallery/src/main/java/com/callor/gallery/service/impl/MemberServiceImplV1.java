...
public class MemberServiceImplV1 implements MemberService{
	 ...
	 * 
	 * 2. ADMIN 권한을 갖는 최초의 가입가는 level 0 이다
	 ...
	 * 7. 이미 가입된 member의 m_userid 정보가 JOIN을 통해서 전달되면 회원 정보를
	 * 		update한다
	 */
	@Override
	public MemberVO join(MemberVO memberVO) {
		...
		// 아직 member table에 데이터가 하나 없는 상태 = 최조 가입 멤버 = ADMIN = 0
	...
	public MemberVO findById(String m_userid) {
		...
		if(memberVO == null) {
			...
			log.debug("사용자 정보 {}", memberVO.toString());
		}
		return memberVO;
	}
	@Override
	public MemberVO login(MemberVO memberVO, Model model) {
		...
		//	 	만약 결과가 null 이면 : 가입 ㄴㄴ
		// 		결과가 null 아니면 : 비밀번호 일치 조회
		// 3. 비밀번호 일치 조회
		// 		일치하지 않으면 : 비밀번호 오휴 로그인 거부
		//		일치하면 : 로그인 처리
